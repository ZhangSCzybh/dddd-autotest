...
public class LoginUtil {
    ...
    //账号密码登录商城
    public static String loginFuliMallToken(String loginUrl,String userinfo) {
        try {
            // 1. 创建 URL 对象
            URL url = new URL(loginUrl);
            // 2. 打开 HttpURLConnection 连接
            ...
            try (OutputStream os = connection.getOutputStream()) {
                // 将请求体数据写入输出流
                byte[] input = userinfo.getBytes("UTF-8");
                //byte[] input = ("{ \"selfOperatedPlatform\": \"1\" ,\"password\": \"123123123\",\"loginName\": \"abc034\",\"agreePrivacyPolicy\": \"1\"}").getBytes("utf-8");

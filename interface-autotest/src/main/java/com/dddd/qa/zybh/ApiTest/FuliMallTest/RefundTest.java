...
public class RefundTest {
    ...
    @Test(description = "下单平台供应商，福粒通用积分")
    ...
    @Test(dependsOnMethods = "purchaseFuliPlatformSku",description = "平台供应商单商品，提交待发货退款")
    public void fuliPlatformSkuRefund(){
        ...
        // 获取第一个订单（rows[0]）
        JSONObject firstOrder = rows.getJSONObject(0);
        ...
        Config.itemOrderNumber = firstItemOrder.get("itemOrderNumber").toString();

        Assert.assertNotNull(Config.itemOrderNumber, "第一个订单的子订单编号不为空");
        CommonUtil.assertAvailable(jsonresult, body, createUrl,Config.FuliYunYingPro, scene2);
        logger.info("第一个订单的子订单编号: id={}",Config.itemOrderNumber);


        JSONObject param2 = JSONUtil.createObj();
        param2.put("itemOrderNumber", Config.itemOrderNumber);
        param2.put("refundApplyProblemId", 40);
        param2.put("reason", "测试待发货退款test");
        String body2 = param2.toString();
